::
::   Jul   14, 2022 :: Ronald F. Premo III :: fixed header for GAMESS Boston.
::
::   Apr   25, 2022 :: Sarom Leang :: update script for GAMESS 30 June 2021 R2 Patch 2
::
::   Dec   04, 2020 :: Sarom Leang :: update script for GAMESS 30 Dec 2020 R2
::
::   Aug   09, 2019 :: Sarom Leang :: update script for GAMESS 30 June 2019 R1 Patch 1
::
::   Mar   08, 2015 :: Sarom Leang :: change parameters.gms to rungms.gms
::
@REM
...
@REM [DEFAULT:FALSE] = NO REDIRECTION.
@REM
...
@REM [1] The ability to allow the user to use the rungms.gms file to
...
@REM
@IF NOT EXIST rungms.gms (
  @ECHO -------------------------------------------------------------------------
  @ECHO "Oh no you didn't!"
  @ECHO ERROR :: rungms.gms file not found. Please make one.
  @ECHO =========================================================================
  @ECHO.
  @ECHO Create a new txt file called rungms.gms in this directory.
...
@REM
@FOR /F "tokens=1,2 delims==" %%A IN (rungms.gms) DO @SET %%A=%%B
@REM
...
@ECHO.
@ECHO GAMESS References:
@ECHO.
@ECHO "General Atomic and Molecular Electronic Structure System"
@ECHO M.W.Schmidt, K.K.Baldridge, J.A.Boatz, S.T.Elbert, M.S.Gordon,
@ECHO J.H.Jensen, S.Koseki, N.Matsunaga, K.A.Nguyen, S.Su, T.L.Windus, M.Dupuis,
@ECHO J.A.Montgomery J. Comput. Chem., 14, 1347-1363(1993).
@ECHO.
@ECHO "Advances in electronic structure theory: GAMESS a decade later"
@ECHO M.S.Gordon, M.W.Schmidt pp. 1167-1189, in
@ECHO "Theory and Applications of Computational Chemistry: the first forty years"
@ECHO C.E.Dykstra, G.Frenking, K.S.Kim, G.E.Scuseria (editors),
@ECHO Elsevier, Amsterdam, 2005.
@ECHO =========================================================================
@REM
@REM We print the contents of rungms.gms that may affect this script.
@REM
@IF NOT %SUPRESS%==TRUE (
  @ECHO.
  @ECHO Contents of rungms.gms file:
  @ECHO.
  @FOR /F "tokens=1,2 delims==" %%A IN (rungms.gms) DO @ECHO %%A=%%B
  @ECHO.
  @ECHO =========================================================================
)
@REM
...
@IF NOT DEFINED GAMESSDIR (
  @ECHO -------------------------------------------------------------------------
  @ECHO "Oh no you didn't!"
  @ECHO ERROR :: You need to define GAMESSDIR in rungms.gms
  @ECHO =========================================================================
  @ECHO Now exiting.
  @EXIT /B
)
@IF NOT DEFINED AUXDATADIR (
  @ECHO -------------------------------------------------------------------------
  @ECHO "Oh no you didn't!"
  @ECHO ERROR :: You need to define AUXDATADIR in rungms.gms
  @ECHO =========================================================================
  @ECHO Now exiting.
  @EXIT /B
)
@IF NOT DEFINED RESTARTDIR (
  @ECHO -------------------------------------------------------------------------
  @ECHO "Oh no you didn't!"
  @ECHO ERROR :: You need to define RESTARTDIR in rungms.gms
  @ECHO =========================================================================
  @ECHO Now exiting.
  @EXIT /B
)
@IF NOT DEFINED SCRATCHDIR (
  @ECHO -------------------------------------------------------------------------
  @ECHO "Oh no you didn't!"
  @ECHO ERROR :: You need to define SCRATCHDIR in rungms.gms
  @ECHO =========================================================================
  @ECHO Now exiting.
  @EXIT /B
)
...
@IF [%1]==[] (
  @ECHO -------------------------------------------------------------------------
  @ECHO.
  @ECHO Usage from the COMMAND PROMPT: rungms.bat [input] [version] [ncpus] [logfile]
  ...
  @ECHO  [logfile] = If a 4th argument is passed then the output of the GAMESS run is
...
@REM
@IF NOT EXIST %JOBPATH% (
  @IF NOT EXIST %JOB%.inp (
    @REM
    @REM In the case of EXAMnn jobs, this file might be in the "tests" subdirectory.
    @REM
    @SET "NOTFOUND="
    @IF EXIST %GAMESSDIR%\tests\travis-ci\%JOB%.inp (
      @REM
      @REM The file exists in the test directory. Copy it over to the local scratch.
      @REM
      @COPY /Y %GAMESSDIR%\tests\travis-ci\%JOB%.inp %SCRATCHDIR%\%JOB%.F05 > NUL
    ) ELSE (
      @SET NOTFOUND=1
    )
    @IF EXIST %GAMESSDIR%\tests\travis-ci\parallel\%JOB%.inp (
      @REM
      @REM The file exists in the test directory. Copy it over to the local scratch.
      @REM
      @COPY /Y %GAMESSDIR%\tests\travis-ci\parallel\%JOB%.inp %SCRATCHDIR%\%JOB%.F05 > NUL
      @SET "NOTFOUND="
    ) ELSE (
      @SET NOTFOUND=1
    )
    ...
    @REM
    @REM The file exists in the given path. Copy it over to the local scratch.
    @REM
    @COPY /Y %JOBPATH% %SCRATCHDIR%\%JOB%.F05 > NUL
)
@IF NOT %SUPRESS%==TRUE (
  @ECHO.
  @ECHO Job Properties
...
@SET  EXTBAS=/dev/null
@SET  NUCBAS=/dev/null
@SET  EXTCAB=%AUXDATADIR%\AUXBAS\auxdef21
@SET  POSBAS=/dev/null

@SET DFTBPAR=%AUXDATADIR%\DFTB
...
@SET  QMWAVE=%RESTARTDIR%\%JOB%.qmw
@SET   MDDIP=%RESTARTDIR%\%JOB%.dip
@SET OPTHES1=%RESTARTDIR%\%JOB%.hs1
@SET OPTHES2=%RESTARTDIR%\%JOB%.hs2
@SET FCIDUMP=%RESTARTDIR%\%JOB%.FCIDUMP

@SET     XYZ=%SCRATCHDIR%\%JOB%.xyz
...
@SET     UNV=%SCRATCHDIR%\%JOB%.F45
@SET    UNPV=%SCRATCHDIR%\%JOB%.F46
...
@REM
@REM Next values for for spin-space 2nd order density.
@REM
@SET  DEN2P1=%SCRATCHDIR%\%JOB%.F70
@SET  DEN2P2=%SCRATCHDIR%\%JOB%.F71
@SET  DEN2P3=%SCRATCHDIR%\%JOB%.F72
@SET  DEN2P4=%SCRATCHDIR%\%JOB%.F73
@REM
@REM Next files are for spatial no-migration 2nd order density
@REM
@SET  DEN2NM=%SCRATCHDIR%\%JOB%.F74
@SET DEN2OPT=%SCRATCHDIR%\%JOB%.F75
@REM
...
@REM
@REM Next files are used for explicitly correlated methods
@REM
@SET R12INP=%RESTARTDIR%\%JOB%
@SET PT2BAS=%SCRATCHDIR%\%JOB%.F300
@REM
...
@REM
@REM CASINO trial wavefunction file
@REM
@SET CASINO=%RESTARTDIR%\%JOB%.casino
@REM
...
@IF EXIST %OPTHES1% (
  @IF %ERASEOLDFILES%==TRUE (
    ...
    @ECHO Found old OPTHES1 : %OPTHES1%
    @SET ERROR=TRUE
  )
)
@IF EXIST %OPTHES2% (
  @IF %ERASEOLDFILES%==TRUE (
    ...
    @ECHO Found old OPTHES2 : %OPTHES2%
    @SET ERROR=TRUE
  )
)
@IF EXIST %XYZ% (
...
@IF EXIST %COSDATA% (
  @IF %ERASEOLDFILES%==TRUE (
    ...
    @ECHO Found old COSDATA : %COSDATA%
    @SET ERROR=TRUE
  )
)
@IF EXIST %COSPOT% (
...
@IF EXIST %CIMFILE% (
  @IF %ERASEOLDFILES%==TRUE (
    ...
    @ECHO Found old CIMFILE : %CIMFILE%
    @SET ERROR=TRUE
  )
)
@IF EXIST %CIMDMN% (
...
@REM
@REM Source Intel MPI environment
@REM
...
@IF %REDIRECTION%==TRUE (
  ...
  @REM
  @REM @DIR %SCRATCHDIR%\%JOB%.* | FIND %JOB%
  @REM
  @ECHO  ----------------------------------------
